...
8. For long runs, add `--session-file .wfm/session.json` and use `wfm follow --session-file .wfm/session.json` from another terminal. Add `--follow` to the run command to open that follower in a new Herdr tab or tmux window. Every run keeps a durable JSONL transcript under `~/.workflow-manager/runs/` for later diagnosis.
