...
7. Run `wfm run <workflow-file>` to watch live progress in the CLI. Its final result is JSON on stdout by default, so another CLI or coding agent can consume it; use `--text` for a human-oriented final summary. Human approvals now show an inline terminal review prompt, and `--verbose` streams per-step agent output.
8. For long runs, add `--session-file .wfm/session.json` and use `wfm follow --session-file .wfm/session.json` from another terminal. Add `--follow` to the run command to open that follower in a new Herdr tab or tmux window. It shows assistant deltas, tool activity, stdout, and stderr as they arrive; Pi's prompt/input/output/skill/native-session files are retained under `.wfm/agents/`. Every run keeps a durable JSONL transcript under `.wfm/runs/` for later diagnosis. The follower is a live activity stream rather than a second native agent TUI.
