...
**Approval-step precedence:** an unset step-level `validation` defaults to `{ mode: "none", required: false, autoConfirm: true }`. On `approval` steps, the engine resolves `mode` and `autoConfirm` the same way: `approvalSpec.validation` wins whenever it sets `required: true`, so `approvalSpec.validation` alone is enough to gate an approval step on a human decision — you don't need to duplicate a matching top-level `validation` block just to avoid a silent auto-approve. Setting both (with matching `mode`/`required`/`autoConfirm`) is still fine and reads clearly, but it is no longer required for correctness. See [Authoring Workflows With an Agent](/guide/authoring-with-an-agent#approvals-human-decides-or-the-agent-decides-for-you) for a worked example.
