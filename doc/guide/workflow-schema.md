...
- `taskSpec.adapterKey`: optional; omitted task adapters run with `pi-agent`. Explicit values are `pi-agent | mock | acp | opencode | codex | claude-code | kimi | gemini | qwen`.
...
- ACP payload fields (for `acp` and the ACP-routed `claude-code | opencode | codex | kimi | gemini | qwen`):
  - `taskSpec.payload.useRealAdapter`: for `acp`, `claude-code`, `codex`, `kimi`, `gemini`, and `qwen`, set `true` to run the agent through ACP (otherwise the step mocks). `opencode` is the exception — it runs real by default through ACP; set `false` to opt out to the mock executor.
  - `taskSpec.payload.acpCommand` / `acpArgs`: explicit ACP agent command and args
  - `taskSpec.payload.acpAgent`: a preset name (`claude-code | opencode | gemini | codex | kimi | qwen`) when `adapterKey` is `acp`
...
- `agent.adapterKey`: optional; one of the supported adapters (`pi-agent | mock | acp | opencode | codex | claude-code | kimi | gemini | qwen`). Defaults to the validated step's own `taskSpec.adapterKey` when omitted.
...
- `acp`: Agent Client Protocol adapter; connects to any ACP agent over JSON-RPC/stdio when `useRealAdapter` is true and an agent command resolves. Verified against `gemini --acp`; set `payload.acpAgent: gemini` (or `acpCommand`/`acpArgs`).
...
- `kimi`: routed through ACP via the `kimi` CLI's native ACP mode (`kimi acp`) when `useRealAdapter` is true. No bridge package needed — Kimi speaks ACP directly, the same way `opencode`/`gemini` do. It reuses the Kimi CLI's own auth (`/login` inside the CLI).
- `gemini`: routed through ACP via the official Gemini CLI's native ACP mode (`gemini --acp`) when `useRealAdapter` is true. No bridge package needed. Install with the Gemini CLI (npm package `@google/gemini-cli`); it reuses the Gemini CLI's own auth.
- `qwen`: routed through ACP via the official Qwen Code CLI's native ACP mode (`qwen --acp --experimental-skills`) when `useRealAdapter` is true. No bridge package needed. Install with Qwen Code (npm package `@qwen-code/qwen-code`); it reuses the Qwen Code CLI's own auth.
