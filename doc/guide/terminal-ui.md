...
- `o` opens a live agent-activity follower in a new Herdr tab or tmux window — it runs `wfm follow --session-file <path>`. The follower streams assistant deltas, tool activity, stdout, and stderr, rather than attaching a second native agent TUI to WFM's headless subprocess. When `--ui` is started without an explicit `--session-file`, a file like `.wfm/session-<run-id>.json` is created automatically so the follow key always works. Herdr or tmux must be available; otherwise the status line explains what is missing.
