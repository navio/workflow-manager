...
Use `wfm doctor` to inspect host adapter setup and `wfm doctor <workflow>` to validate a workflow's runtime requirements before running it. Today, `pi-agent` is the real default host adapter (driving the `pi` CLI), `mock` is a deterministic simulator, `opencode` runs real by default through ACP (`taskSpec.payload.useRealAdapter: false` opts a step out to mock), and `claude-code`, `codex`, and `kimi` have opt-in real paths through ACP (`useRealAdapter: true`). Codex routes through the `codex-acp` bridge — install it with `npm install -g @agentclientprotocol/codex-acp`; it reuses the codex CLI's own auth, so no API key environment variable is needed. Kimi routes through the `kimi` CLI's native ACP mode (`kimi acp`) — no bridge package is needed, since Kimi speaks ACP directly; it reuses the Kimi CLI's own auth (`/login` inside the CLI).
