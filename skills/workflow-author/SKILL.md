...
- key: my-step                     # stable, kebab-case
  ...
  validation:                      # gates confirmation for THIS step's own record
    ...
    agent:                         # only when mode: agent
      adapterKey: pi-agent | mock | opencode | codex | claude-code | kimi | gemini | qwen | acp   # default: this step's adapter
  ...
  taskSpec:                        # required when kind: task
    adapterKey: pi-agent | mock | opencode | codex | claude-code | kimi | gemini | qwen | acp    # omit -> pi-agent
    init:
